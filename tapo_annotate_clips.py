...
⚠️ **2026-09-03 實測結論：這批片段做不到第 1 步。**
   30 支標完之後：人工 60 次動作，演算法在最寬鬆的門檻只抓到 35 次、
   誤報僅 2 次 —— 每個門檻都嚴重漏報，而且誤差隨門檻單調遞減
   （最佳值永遠落在格點端點，那是假的最佳）。

   真正的原因是選擇偏誤：**片長是舊偵測器自己決定的**（它看到更多翻身
   就延長錄影），所以片長已經編碼了「那段有多熱鬧」。實測片長單獨與
   人工次數 r=0.84，控制片長之後最好的度量只剩偏相關 0.33、
   事件數幾乎歸零（0.04）。
...
def cmd_compare():
    ...
    print("-" * 84)
    print(f"\n誤差最小：{best[0]}% 畫面（誤報+漏報 = {best[1]} 次）")
    if best[0] in (GRID[0], GRID[-1]):
        print(f"  ⚠️ **但那是掃描格點的端點** —— 誤差還在單調下降，")
        print("     真正的最小值在格點之外。**不要把它當成校準結果。**")
    _confound_check(both, truth, meas)
    print("""
  ⚠️ 誤報/漏報那張表**單獨不足以定案** —— 一定要配上面的混淆檢查一起讀。

  ⚠️ 這批片段有兩個先天限制：
     1. 是**舊偵測器判定為大翻身時**才錄的，所以樣本偏向「有動作」的時刻。
        量得到準確率，量不到漏報率（偵測器完全沒反應時沒有影片）。
     2. 是 stream1（1920x1080），即時記錄是 stream2。兩者都降到
        640x360 才處理，但壓縮假影可能不同。""")
    print("=" * 84)



def _confound_check(clips_, truth, meas):
    """
    ⚠️ 這是這支工具最重要的一段。

    片段是**舊偵測器自己決定要不要錄、錄多久**的：它看到更多翻身就延長錄影。
    所以「片長」已經編碼了「那段時間有多熱鬧」，而人工次數與演算法度量
    兩邊都只是同一件事的代理 —— 樣本被待評估的東西自己挑過。

    實測（n=30）：片長單獨與人工次數 r=0.84；控制片長之後，最好的度量
    只剩偏相關 0.33，事件數幾乎歸零（0.04）。
    """
    import numpy as np
    h = np.array([float(truth[c]["n_movements"]) for c in clips_])
    ln = np.array([float(meas[c]["frames"]) for c in clips_])
    if len(h) < 8 or h.std() == 0 or ln.std() == 0:
        return
    r_len = float(np.corrcoef(ln, h)[0, 1])
    print()
    print("【混淆檢查】片長能不能解釋掉一切")
    print("-" * 84)
    print(f"  片長 vs 人工次數：r = {r_len:.2f}")

    def partial(x):
        rx = x - np.polyval(np.polyfit(ln, x, 1), ln)
        rh = h - np.polyval(np.polyfit(ln, h, 1), ln)
        return float(np.corrcoef(rx, rh)[0, 1]) if rx.std() else 0.0

    rows = []
    for q in GRID:
        a = np.array([float(meas[c][f"n@{q}%"]) for c in clips_])
        rows.append((q, float(np.corrcoef(a, h)[0, 1]) if a.std() else 0.0, partial(a)))
    print(f"  {'門檻':>7}{'原始 r':>10}{'控制片長後':>12}")
    for q, r0, rp in rows:
        print(f"  {q:>6.1f}%{r0:>10.2f}{rp:>12.2f}")

    best_partial = max(abs(rp) for _, _, rp in rows)
    print("-" * 84)
    if abs(r_len) > 0.6 and best_partial < 0.5:
        print("  ⚠️ **這批樣本不能用來校準門檻。**")
        print(f"     片長單獨就解釋了 r={r_len:.2f}，控制片長之後最好的度量只剩 "
              f"{best_partial:.2f}。")
        print("     片段由舊偵測器自己挑選並決定長度 —— 這是選擇偏誤。")
        print()
