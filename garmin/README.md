...
# 連 Garmin 重新抓資料再算全部
python run_pipeline.py --fetch --days 30
```

最終結果在 `data/garmin_sleep_quality_final.csv`。
...
| ⑤ | `apply_recovery_modifier.py` | Tier3 修正值（±12）+ SRI（呈現用不計分），產出最終分數 |
...
- **`movement_count` 是我們自訂的取樣筆數**，不是 Garmin 顯示的數值，不要拿去跟 App 對照。
