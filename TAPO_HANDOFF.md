...
⚠️ 附帶：`motion_intensity == 2073600`（整畫面）有 238 筆，
全部落在 `large_turn`（1017 筆中的 23.4%），micro 一筆都沒有。
那是另一個獨立的誤判問題，值得修，但**修了不會改變分數的行為**——
撐起事件量的是 micro。
...
| ② 照明否決 | 直接解掉 `motion_intensity = 2073600` 那 238 筆（large_turn 的 23.4%） |
