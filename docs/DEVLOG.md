...
但**一直沒有任何東西寫進去**。這一輪把來源接上：手機事件流 → `lights_out_at`
→ `POST /nightly` → 挑戰進度。

### 這一輪最重要的內容：三個單元測試看不見的錯

演算法寫完、89 條測試全綠、commit 也進去了，插上手機卻回報「偵測不到就寢時刻」。
拉 `adb shell dumpsys usagestats` 當獨立來源對照才找到根因——**三個錯疊在一起**，
而且每一個在畫面上都長得像正常輸出（沒有 crash、沒有錯誤訊息、沒有可疑數字）。

**① 螢幕亮 ≠ 有人在用。** 2026-09-01 03:45~08:00（人明顯在睡）：

| 事件 | 次數 |
|---|---|
| `KEYGUARD_HIDDEN`（真的解鎖） | **0** |
| `SCREEN_INTERACTIVE` | 14 |
| `ACTIVITY_RESUMED` | 14（Bixby 6、抖音 6、鬧鐘 2） |

全是通知把螢幕點亮與背景活動。那些假互動把整夜切成 39/30/28 分鐘的碎片，
最長的安靜期只剩 **148 分鐘**而且落在白天。改成「解鎖才算互動」之後，
同一份資料的最長安靜期變成 **249 分鐘**（03:51 放下 → 08:00 拿起），
第二長的 148 分鐘，兩者分得很開。

順帶解掉另一個問題：解鎖區段本來就涵蓋「連續看兩小時同一個 App」，
activity 模式要靠 `resumed→paused` 配對才補得起來的洞，unlock 模式根本不存在。

**② `limit=2000` 把視窗砍掉一半。** 實機 24 小時有 **4057 筆**事件（三星 One UI，
通知多），而截斷丟的是**舊的**那半，正好是昨晚睡覺那段。提高到 20000
（≈5.7 天餘裕），並在筆數撞到上限時把視窗起點夾到第一筆事件。

**③ 就寢時刻正好是事件流的第一筆。** `queryEvents()` 回來的第一筆就是
`2026-09-01 03:51:11 screen_off`，前面沒有解鎖可配對。「沒配對就丟掉」
（為了擋 ① 的噪音而寫的規則）會把唯一正確的答案丟掉。分界改成：
`keyguard_shown` 沒配對也算（上鎖一定是從解鎖狀態轉過去的），
`screen_off` 沒配對不算（鎖著時通知也會讓螢幕亮了又暗）。
實測那一夜 `KEYGUARD_SHOWN` 只出現在 03:51 一次，訊號是乾淨的。

### 順手釐清：不需要在半夜跑，也不需要背景服務

使用者問「可以在半夜跑這個嗎」。實測回答：

| 查詢視窗 | 筆數 | 最早的事件 |
|---|---|---|
| 2 天 | 6808 | 08-31 07:31 |
| 3 天 | 11774 | 08-30 07:00 |
| 7 天 | 20000（撞上限） | 08-28 13:18 |

約 3500 筆/天，**事件保留 ≥5 天**。所以早上開 App 就回推得到昨晚；
而且演算法本來就要求「那段安靜已經結束」，半夜跑的時候人還在睡，
結構上得不到答案。後端也是為事後上傳設計的（日期來自
`night_date(lights_out_at)` 而不是「現在」，`upsert` 冪等）。

→ 背景排程（WorkManager）**不做**：5 天保留期讓「忘記開 App」有很大容錯，
而三星會殺背景、各家 ROM 行為不同，09-09 之前不值得投。

### 實機實測（2026-09-03 01:16 ~ 01:35）

```
LightsOut[unlock] ok raw=2574 used=497 quiet=329m at=2026-09-02 02:39:18
...
```
nightly_behavior: 3 列 / 2 人
late_night_ratio 1.0, bedtime_spread_minutes 36.0
GET /challenges → "Lights out on time": recorded_nights 1, "261 min later than your target."
```

189 分鐘 = 23:30 目標 → 02:39 實際。跨午夜算對了（Dart 若自己相減會得到
「提早 20 小時 51 分」）。**`bedtime_spread_minutes` 是「作息收斂」挑戰的標的，
第一次有真實資料在算。**
...
| build | 結果 |
|---|---|
| 帶 `SONNAP_USER_ID` | 不問暱稱、直接上傳 |
| 不帶 | 跳出註冊 → 建立「Nathan」(cohort L0) → 上傳成功 → **重開不再問、也沒有第二個帳號** |

四態寵物在實機上確認了 happy（Aug 23 / Jul 2）、anxious（**Jul 6，品質是 Good**
——「不能從 quality 推 mood」那條紅線的活證據）、bored（Aug 29）。
tired 那一下沒點到（手機被拿去用了），四態的資產本身是同一支腳本產生的。

### 踩到的坑

**`TestWidgetsFlutterBinding` 會把全域 `HttpClient` 換成「一律回 400」的假實作。**
只要同一個檔案裡有任何一條 `testWidgets` 就會裝上。症狀是連 `127.0.0.1:1`
（根本沒有東西在聽）都「成功」拿到 400 而不是丟 `SocketException`，
而錯誤訊息完全沒有指向 `HttpOverrides`。
`nightly_uploader_test.dart` 沒中招純粹是因為它整支沒有 `testWidgets`。

**`_FakeUsageStats` 沒攔截新加的 `lightsOut()`**，widget test 打到真的
MethodChannel 卡住，失敗訊息卻長得像「找不到 App 名稱」，指向完全錯的地方。

**自己寫的守門測試被自己的新功能撞到。** 前一輪寫了「畫面不得出現
`past your target`」來守「Dart 不准自己算達成度」。這一輪後端真的算了、
畫面要顯示，那條測試就紅了。改寫成「**沒有後端回應時**不得出現」並補一條
反向對照——少了反向對照的話，把整段達成度拿掉、永遠不顯示，測試仍然全綠。

**帳號功能做完才看見的舊 bug**：三個畫面各自寫死 `"Jeremy"`（隊友的名字）。
用「Nathan」註冊完，首頁還是說「Good morning Jeremy」。
...
|---|---|
| 測試 | 72 → **120** |
| `flutter analyze` | 3 個 issue（都是既有的未使用常數，未增加） |
| commit | 5 個，PR #30 |
| 新檔 | `lights_out.dart`、`nightly_uploader.dart`、`user_identity.dart`、`account_service.dart`、`key_value_store.dart`、`onboarding_screen.dart`、`KeyValueStore.kt` |

⚠️ 合併 `origin/main`（PR #29，Garmin 補抓到 09-01）之後 `app_payload.json`
換了 5 晚，history 的 30 晚窗格整個滑動。釘住特定夜晚的驗收測試
（07-06 anxious 等）**剛好還在窗內——那是運氣不是設計**，再補幾晚就會掉出去。
