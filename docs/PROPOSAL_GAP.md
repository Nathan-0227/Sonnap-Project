...
> 建立日期：2026-08-18｜**最後逐條重驗：2026-09-10**｜撰寫者：陳泰銘（AI 與數據處理）
>
...
>
> ### 狀態標記怎麼讀（2026-09-10 起）
>
> | 標記 | 意思 |
> |---|---|
> | ✅ | **在 `origin/main` 上**，可以直接驗證 |
> | 🟡 | 部分完成，或**已實作但卡在未合併的分支** —— 後者一律註明分支名與 commit 範圍 |
> | 🔴 | 零程式碼 |
>
> ⚠️ 「分支上做好了」**不等於做完了**。功能沒進 main 就進不了給受測者的 APK，
> 對 D2 而言等於不存在。這一版有四項處於這個狀態（全部在 `feature/product-loop`）。
>
...
| 2 | 建構**睡眠行為與遊戲獎懲的映射模型**，轉換為經驗值與寵物健康指標 | 🟡 **獎懲有了，數值化沒有**。「分數 → 寵物心情」四態可用；挑戰引擎（**4 個**行為挑戰）在後端跑著。但沒有經驗值、沒有貨幣、沒有成長 | 有的部分：`build_app_payload.py` `QUALITY_TO_MOOD`；`behavior/challenges.py`（463 行）＋ `GET /challenges`。沒有的部分：`xp`／`experience`／`coin`／`currency`／`achievement` 在 `app/lib/` **仍是零命中**（`level` 只命中 Garmin 的 `levelMean`／`levelMax`）。⚠️ **挑戰進度後端算好了，main 上的 App 一個畫面都沒有**——`challenge` 在 `app/lib/` 唯一命中是 `user_identity.dart:9` 的一行註解。⚠️ 挑戰進度卡**已在 `feature/product-loop` 實作**（`fdb39fb`，三條紅線各有一條測試守著），尚未合併 |
...
| # | 計畫書寫的 | 現況 | 證據 |
|---|---|---|---|
| 1 | **使用者需求分析**：行為調查問卷、深度訪談、App Store 負評文本分析 | 🟡 **2026-09-06 補上了**（`Research-Background/需求分析.md`）：競品負評實證、流失原因、強制阻斷 vs 自我推力、14 題行為調查問卷、11 題訪談大綱、6 條可驗證的設計結論。⚠️ **尚未實際發放**，零筆回收 | ⚠️ **負評分析沒有自己爬 App Store**（沒有公開 API、抓取違反 ToS，且學生專題的樣本量做不出可靠的主題分析）。改用 Nuo 等人（2023）已發表的 **488,071 則評論**分析，樣本大三個數量級且經同儕審查——**但代價是樣本不含台灣大學生、也不含遊戲化睡眠 App**，該文件逐節標了證據等級 |
...
| 3 | **使用者評估**：多名大學生安裝實測、事後訪談 | 🔴 **仍未開始**（零筆受測者資料） | 材料已備齊於 [D2_STUDY/](D2_STUDY/)：同意書／隱私政策／招募文／前後測問卷／操作手冊。三個結構性阻礙也已解除（見第二節）。**卡的只剩招募** |
...
| 目標就寢時間可設定 | 🟡 **能選、兩頁同步了，但仍不存檔** | 兩頁共用同一份 state：`main.dart:52` 是唯一擁有者（PR #28）。⚠️ 但那一行自己標明「**目前只存在記憶體裡，App 關掉就回到預設值**」。持久化的位置就是那裡，`KeyValueStore`（`sonnap/store`）現成可用 ⚠️ **`feature/product-loop` 上已實作但尚未合併**（`user_settings.dart`）（`4a48e7a`~`756f7b1`，5 個 commit）。合併前 main 上仍是這個狀態。 |
| **接近就寢時間，時間字體轉紅色** | 🟡 **main 上沒有；分支上做好了** | 09-06 重驗：`header_card.dart` 全檔 `Colors.red`／依剩餘時間的顏色分支**零命中**。所有顏色都是編譯期常數 ⚠️ **`feature/product-loop` 上已實作但尚未合併**（`4a48e7a`~`756f7b1`，5 個 commit）。合併前 main 上仍是這個狀態。 |
...
| 睡眠計量條，隨最近幾天熬夜程度變動 | 🟡 **後端算好了；畫面在分支上** | streak 仍是「連續有睡眠**記錄**的夜數」，衡量的其實是配戴率（`compute_streak()` docstring 自己標明）。但真正的熬夜比率**後端已經在回了**：`adherence.late_night_ratio()`，`main.py:488`（`/home`）與 `:604`（`/insights`）都有 `late_night_ratio`／`late_nights`。**main 上的 App 沒有任何地方顯示它**。⚠️ **`feature/product-loop` 上已實作但尚未合併**（`4a48e7a`~`756f7b1`，5 個 commit）。合併前 main 上仍是這個狀態。 |
...
| **熬夜比率（圖五那個 40%）** | 🟡 **後端有；畫面已實作但未合併** | `behavior/adherence.py` 的 `late_night_ratio()`；`/home` 與 `/insights` 都在回。App 端零命中——**接上去就有，是目前最便宜的補洞之一** |
| **熬夜時最常使用的前三項 App 及佔比** | ✅ **做完了**（PR #37／#39） | 09-06 記的「還不是睡前、只有日彙總」**已不成立**：現在真的切到**上床前 60 分鐘**（`preBed()` 走 `queryEvents` 依 App 切段，`report_screen.dart:149`），偵測不到就寢時刻時才退回日彙總，**標題與說明文字會跟著換**。`pre_bed_apps_test.dart` 守著三個寫錯不會報錯的地方：未配對的 `resumed` 要延續（拿著手機睡著）、螢幕關閉要關掉區段、區段要與視窗**取交集**不是整段算 |
...
| 項目 | 計畫書要求 | 實際做到 |
|---|---|---|
| 睡眠評分 | 「客觀睡眠數據」（無方法要求） | Tier1/2 文獻加權（每項都有 DOI）＋ Tier3 個人化修正 ＋ SRI，**64 晚**實測（2026-05-29 ~ **09-08**）。`Research-Background/Garmin手錶分數.md` 383 行 A–M 節論述。⚠️ 那 64 晚**跨 3 名配戴者**（41／11／12），而且**研究者本人那 12 晚沒有任何一晚有 Tier3 修正值**（12 < `MIN_BASELINE_NIGHTS` 14）——報告不能寫「三層評分全數運作」。見 [REPORT.md](REPORT.md) 與 [REPORT_CAVEATS.md](REPORT_CAVEATS.md) |
| LLM 串接（C1） | 「LLM API 串接與 Prompt 測試」 | **64 晚全數由 LLM 生成、0 晚 fallback**（09-10 補齊） ＋ **四道確定性驗證層**（夢境不得出現數字、建議的數字須逐字存在於事實區塊、醫療宣稱禁詞、「無記錄」意象只准用於真的沒測到的夜晚）。`PROMPT_VERSION` 已到 **v4** |
...
2. **一到兩週的研究，Tier3 全部處於冷啟動**（需 14–28 晚個人 baseline）；
   **SRI 也算不出來**（需 28 天窗格 ≥10 組相鄰配對——研究者自己 64 晚
   也只有一部分算得出來）
...
| **縱貫基準** | 研究者本人 | **64 晚** ＋ 攝影機 ＋ Tier3 ＋ SRI | 不可替代。⚠️ 那 64 晚**跨 3 名配戴者**，可確認同一人的最長連續區段是 41 晚 |

⚠️ **Tier3 與 SRI 需四週以上才長得出來，受測者無論如何都不會有。
研究者那 64 晚本來就是全隊唯一一份縱貫資料，不需要靠外借去複製它。**
...
| 小米手環 9 Active NT$579 | 64 晚資料實際來自 **Garmin Vivoactive 3**（研究者自有） |
...
| 查核點 | 原訂日期 | 權重 | 現況 |
|---|---|---|---|
| A1 文獻探討與競品功能分析 | 2026/04/01 | 5% | 🟡 **文獻與競品分析都有了，問卷尚未發放**。`Garmin手錶分數.md` 383 行逐項 DOI ＋ `攝影機分數.md` ＋ **`需求分析.md`（09-06 新增）**。後者含競品負評實證（4 條引用全核對第一作者）、行為調查問卷、訪談大綱。**缺的只剩實際發放與回收** |
| A2 UI/UX 介面設計與原型製作 | 2026/05/16 | 10% | ✅ 五個頁面切版完成 |
| B1 前端 APP 介面切版與邏輯 | 2026/07/01 | 15% | 🟡 切版完成；**五頁中四頁接了真資料**（Home／Insights／Assistant／Settings），只剩 `friends_screen` 全寫死。09-10 補：**睡前 60 分鐘 App 分布**與**上床／下床標記**已在 main（PR #37／#39／#40）；挑戰進度卡、熬夜比率、就寢時間持久化、倒數變紅**已實作但卡在 `feature/product-loop` 未合併** |
...
| C1 LLM API 串接與 Prompt 測試 | 2026/09/24 | 15% | ✅ **提前約六週完成**（2026-08-12），含四道確定性驗證；**64 晚全數 LLM 生成、0 晚 fallback**。⚠️ 驗證層真的擋過東西：09-05 那晚第一次因「advice 出現事實區塊裡沒有的數字」被拒、重試才過。⚠️ 計畫書目標一要的是**對話**，這一項交付的是單向生成，見 1.1 第 1 列 |
...
| D1 系統整合測試與除錯 | 2026/10/01 | 5% | 🟡 **回歸測試有了，整合測試沒有**。**10 支 Python ＋ 16 支 Flutter 測試檔（168 條）**，多條用「把 bug 重新引入、確認測試會紅」驗證過。但沒有跨模組的端到端自動化測試 |
