...
| `MIN_BLOB_FRACTION` | **ROI 的 0.75%** | ⚠️ 這一格原本寫「5%」，是猜的，而且錯了一個數量級。兩晚人工標註實測：單看每一晚最佳是 1.5–2%（第 3 晚）與 0.75%（第 4 晚），**取兩晚都不差的那個值是 0.75%**（0.89 / 0.86；改用 1.5% 會變成 0.96 / 0.77）。5% 在兩晚召回都掉到 75% 以下。仍然要用自己的影片驗 |
