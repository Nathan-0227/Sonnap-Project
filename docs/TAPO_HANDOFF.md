...
#### ⚠️ 方案 B 我們實際做了，結果是：**它會選錯，而且選錯比不用還糟**

2026-09-06 用兩晚連續錄影 + 逐幀人工標註實測（`tapo_roi_experiment.py`）。

方案 B 描述的「整晚熱區圖取最大連通區塊」就是 `tapo_derive_roi.py`。
問題在於熱區圖**分不出那些動作是誰的**。第 4 晚畫面裡有室友，
推出來的框整個偏掉，把它套到第 3 晚：

| 框從哪來 | 套到第 3 晚 | 套到第 4 晚 |
|---|---|---|
| 整畫面（不用 ROI） | F1 **0.92** | F1 **0.73** |
| 第 4 晚熱區推的（受室友污染） | F1 0.87 ⬇ | — |
| 第 3 晚熱區推的（乾淨） | — | F1 **0.86** ⬆（精確率 65%→80%、誤報 8.1→4.1 次/時） |

第 6 晚是**錄的時候就套 ROI**（`--roi`），框一樣來自第 3 晚，是第二次出樣本驗證：
人工事件率 12.4 次/小時（Montini 常模正中間）、**F1 0.88**（召回 100%、精確 78%）。
整夜 5.78 小時、0 次重連、照明否決 0.2%。

→ **同一個做法，換一晚的資料當輸入，結論相反。** 所以方案 B 一定要補兩件事：

**B′：熱區只用「已知是本人」的幀累積。**
我們用的是人工標註的那些幀（±3 秒窗），影像組沒有標註的話，
至少要用「使用者確認過的那一段」而不是整晚。

**跨晚驗證是必要條件，不是加分項。**
框推出來之後，套到**另一晚**看 F1 有沒有變好。只用同一晚驗＝拿答案對答案。

⚠️ 還有一個對照要做，否則會把「框變小」誤當成「框對了」：
拿**同樣大小、位置不同**的框當控制組。我們做了，
床所在的下半部 +0.12 / +0.08，同樣大小的上半部 +0.01 / −0.02 —— 通過。

#### ⚠️ 更重要的一個發現：ROI 擋不掉大部分的誤報

把熱區圖依人工標記分流成「本人在動」與「其餘時間（＝誤報來源）」兩張，
兩張前 30% 熱區的 **IoU 是 0.47（第 4 晚）與 0.57（第 3 晚）**。
也就是說誤報**大半發生在跟真動作同一塊區域**。

→ 誤報的主要來源**不是**室友或窗簾這種「床以外的東西」，
  而是床上沒被標記的小動作與雜訊。ROI 對這一類完全無效。
→ 所以 ROI 值得做（+0.12 是真的），但**不要期待它把精確率救到 90%**。
  剩下的要靠時間軸那一側（episode 聚合、最小持續時間），不是空間那一側。

⚠️ 沒有 ROI 那一晚的 CSV **事後補不回來**：CSV 只記了「整畫面的最大區塊」，
框外的大動作已經被記成那一塊了。要換分母只能錄的時候就套
（`tapo_metric_logger.py --roi`），或從**影片**重跑整條管線。
...
| `MIN_BLOB_FRACTION` | **ROI 的 0.75%** | ⚠️ 這一格原本寫「5%」，是猜的，而且錯了一個數量級。三晚人工標註實測（下表）。仍然要用自己的影片驗 |
...
三晚人工標註，同一個 ROI `(34,129,288,231)`，每個門檻的 F1：

| 門檻(佔 ROI) | 第 3 晚 | 第 4 晚 | 第 6 晚 | **最差** | 平均 |
|---|---|---|---|---|---|
| 0.50% | 0.79 | 0.79 | 0.88 | 0.79 | 0.82 |
| **0.75%** | 0.89 | 0.86 | 0.84 | **0.84** | 0.86 |
| 1.00% | 0.92 | 0.79 | 0.84 | 0.79 | 0.85 |
| 1.50% | 0.96 | 0.77 | 0.87 | 0.77 | 0.87 |
| 2.00% | 0.96 | 0.83 | 0.87 | 0.83 | **0.89** |
| 5.00% | 0.82 | 0.73 | 0.86 | 0.73 | 0.80 |

**單看任何一晚都會選錯**：第 3 晚會選 1.5%（0.96）、第 4 晚選 0.75%、第 6 晚選 0.5%。
1.5% 在第 3 晚是最高分，但它在第 4 晚是 0.77 —— 那一晚最差的選擇之一。

→ 判準用**最差的那一晚**，不是平均，更不是最好的那一晚。**0.75% 同時是
  「最差最好」與「出樣本最差最好」**（第 3 晚是框的來源，它那一欄偏樂觀，
  排除後仍然是 0.75% 勝出）。2.0% 的平均比較高，但那是被第 3 晚的 0.96 撐起來的。

#### ⚠️ 「動作時長」不要拿去對 Montini 的 4 秒 —— 那是兩個構念

我們量到的時長中位數是 **13–19 秒**，Montini 是 **4 秒**。查過兩種解釋，**都不是**：

| 假設 | 檢驗 | 結果 |
|---|---|---|
| `END_FRAMES` 把好幾次動作黏成一段 | 從 1.3 秒掃到 15 秒 | 時長只從 13.0s 變到 15.5s —— **不是** |
| 背景模型追不上（MOG2 時間常數 100 秒），身體停了還在被判成前景 | 看動作起點後的區塊大小曲線 | 2 秒衝到峰值、維持到 8 秒、12 秒塌掉、16 秒歸零。是**高原後急墜**不是慢衰減 —— **不是** |

→ 那 13 秒是真的有東西在動。差別在**定義**：Montini 數的是人工判讀的離散動作
（有起訖、有拓樸分類），我們數的是「像素面積連續超過門檻」的一整段重新安置。
**兩者不能互相驗證**，時長對不上不代表偵測器壞了。

→ 所以 `tapo_scan_threshold.py` 印的「時長中位數 ✗（Montini 是 4.0 秒）」
  那個記號**只能當提示，不能當驗收條件**。真正的驗收是人工標註算出來的
  precision/recall（`tapo_annotate_continuous.py --compare`）。
