...
**不要自己解析這些檔案。** 根目錄的 [`tapo_index.py`](../tapo_index.py)
是攝影機資料的單一事實來源，同時讀 `sleep_records.sql` 與 `sleep_reports/*/*.json`，
去重、切夜、標出處：

```python
import tapo_index
idx = tapo_index.get_index()                    # {日期: 那一晚的資料}
problem = tapo_index.sleep_recording_problem(n) # None 表示可用
```

實測（2026-09-04）：**索引到 20 晚，19 晚通過有效性判準**。
擋掉的是 2026-06-12——17:26 起、只有 29 秒的白天測試錄影。

⚠️ 那一晚被擋掉不是小事：它是**唯一**讓 TAPO 與 Garmin 的六月資料產生重疊的紀錄。
不擋掉就會憑空多出一個橫跨兩個月的「共同樣本」，
`itegration/if_integrate.py` 的日期範圍會從「08-02 起」被拉回「06-12 起」。
