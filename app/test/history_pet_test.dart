...
void main() {
  ...
  group('⚠️ 每一種心情都必須在某個期間選項下看得到', () {
    ...
    test('30 天的窗格看不到大部分的夜晚——這就是為什麼需要「全部」', () {
      // ⚠️ 這條的前提在 2026-09-12 換過一次，而且是它自己預告的：
      //    舊版釘的是「30 天窗格看不到 anxious」，reason 裡寫明「如果看得到，
      //    這個選項的理由就要重寫」。重抓 Garmin 把資料延長到 09-12 之後，
      //    **7 天窗格就看得到四種心情**，那條斷言因此失效。
      //
      //    現在釘的是不會隨心情分布飄動的那個理由：趨勢圖的期間是**日曆天**，
      //    而 history 是**有資料的夜晚**，兩者不一樣——實測 68 晚橫跨 107 天，
      //    30 天只看得到 22 晚，中間差掉的夜晚在 App 裡打不開。
      final all = withinDays(0);
      final in30 = withinDays(30);
      expect(
        in30.length,
        lessThan(all.length ~/ 2),
        reason: '30 天窗格看不到一半以上的夜晚，所以「全部」這個選項必須存在',
      );
    });
