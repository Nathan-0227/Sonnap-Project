...
/// ## 四個檔目前都在，但其中三個是程式衍生的
///
/// `assets/animations/` 底下四個檔都存在，不過只有 `happy_dog.json` 是美術給的。
/// 另外三個由 `app/tools/derive_pet_moods.py` 從它衍生出來——藏舌頭、垂頭、
/// 瞇眼、縮尾巴振幅、去飽和。**四種狀態是同一隻狗**，這比去網路上抓四隻
/// 不同的狗重要：寵物養成 App 裡心情一變就換一隻狗，產品邏輯本身就是壞的。
///
/// ⚠️ **拿到真正的美術資產之後，直接覆蓋掉那三個檔就好，這個檔一行都不用改**
///    ——`pubspec.yaml` 是以目錄宣告資產的（`- assets/animations/`）。
///    那支 Python 腳本到時候也可以刪了。
///
/// ## 三段式退路仍然留著
///
/// 1. **首選**：`assets/animations/{mood}_dog.json`，照播不套濾鏡。
/// 2. **退路**：檔案不存在時 `Lottie` 的 `errorBuilder` 接手，改播
///    `happy_dog.json` 並**套上該心情的濾鏡**。
/// 3. 連 `happy_dog.json` 都讀不到 → 靜態的爪印 icon。
///
/// 第 2 層現在正常情況下不會走到，留著是因為它幾乎零成本，而且擋的是
/// 「有人刪了某個檔」這種會讓畫面自相矛盾的情況。
///
/// ⚠️ **`fallbackFilter` 的參數與 `derive_pet_moods.py` 的 `MOOD_SPECS`
///    是同一組數字**（同一個 BT.709 色彩矩陣，一個在 render 時套、一個烤進
///    JSON）。改任何一邊都要同步另一邊，否則資產在不在會變成兩種色調。
///    這件事沒有辦法用測試檢查——跨語言。
///
/// ## 合法的心情值
///
