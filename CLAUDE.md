...
⚠️ 後者**通常**較早，但**不是一定**：**滑手機滑到睡著**時，手機要到螢幕逾時
自動鎖定才算「停止使用」，`lights_out_at` 因此落在睡著之後（09-08 晚 55.9 分鐘、
09-11 晚 21 分鐘；原因由使用者 2026-09-12 說明）。`adherence_minutes` 會多報熬夜，
`phone_in_bed_minutes` 會把那段睡眠算成滑手機。四晚實測與兩條可能的修法見
`Research-Background/攝影機分數.md` C 節末——那是產品判斷，還沒決定。
...
`ai/data/ai_advice.json` **64 晚 `source=llm`、0 晚 fallback，但只涵蓋 68 晚中的 64 晚**
（2026-09-12 核對。Garmin 重抓到 09-12 之後，09-09 ~ 09-12 四晚還沒有夢境——
補生成會實際花 API 額度，要先問過再跑）。
...
- **68 晚**實測資料（**2026-05-29 ~ 09-12**，實測自
  ...
  **跨 3 名戴錶者**：
  ...
  | `wearer_c`（08-28 起，負責人本人） | **16** | ✅ |

  ⚠️ **這個數字每次重抓 Garmin 都會變**，寫進報告前跑一次上面那段確認。
  🔴 **Tier3 對負責人本人已經首次啟動**（2026-09-12 重抓後）：16 晚跨過
  `MIN_BASELINE_NIGHTS = 14`，**09-11（−5.73）與 09-12（+1.78）有真正的修正值**，
  08-28 ~ 09-10 那 14 晚仍是「基線建立中、暫停」。
  有修正值的夜晚共 30 晚：28 晚在 `wearer_a`、2 晚是本人。
  ⚠️ 但**報告仍然不能寫「三層評分全數運作」**——2/16 晚不是「運作」，
  而且基線窗格是日曆天（`MAX_BASELINE_DAYS = 28`），配戴一稀疏就會再次停用。
