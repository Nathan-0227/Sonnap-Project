...
⚠️ 這裡**沒有 ROI**，是刻意的：床在哪裡由整夜的動作分布自己說
   （TAPO_HANDOFF 的方案 B），開錄前先框反而是猜。
   為此會另外累積一張**真遮罩**熱區圖，結束時存成 `<檔名>_heat.npy`，
   給 tapo_derive_roi.py 用。CSV 裡的 bbox 是退路——實測光靠矩形
   會把形狀糊掉，熱區沒有自然邊界（百分位一動 ROI 就從 19% 跳到 72%）。
