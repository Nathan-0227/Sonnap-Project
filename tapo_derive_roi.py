...
⚠️ **這支推出來的框不可以直接拿去用。** 它用的是整晚所有動作的熱區，
   分不出哪些是自己、哪些是室友或窗簾。實測第 4 晚（畫面裡有室友）
   這樣推出來的框，套到第 3 晚反而讓 F1 從 0.92 掉到 0.87。
   → 要定框，走 `tapo_roi_experiment.py`：它只用**人工標記的那些幀**
     累積熱區，而且會強迫你跨晚驗證。這支只當快速目視檢查。
...
def main():
  ...
  ⚠️ 這只是等比例換算，不是真的用 ROI 重跑 —— 換算假設「動作大小不變、
     只是分母變小」，但 ROI 真正的作用是讓**框外的雜訊不再有機會成為
     最大區塊**，那件事等比例換算表達不出來。
     要真的用 ROI 有兩條路：
       · 錄的時候套：`tapo_metric_logger.py --roi x,y,w,h`（會多記 roi_* 五欄）
       · 事後從**影片**重跑：`tapo_roi_experiment.py`
     不能只從 CSV 事後除掉 —— 「床外的大動作」在 CSV 裡已經被記成最大區塊了。""")
  ...
  2. 是的話，**先用 tapo_roi_experiment.py 跨晚驗過**，再
     `tapo_metric_logger.py --roi x,y,w,h` 錄下一晚，
     門檻就能改用「佔 ROI 幾 %」表示
