...
這支是目前唯一能替那個誤差**設下界**的路：拿整夜影片人工抽樣標出看得出來
醒著的時段，再回頭跟演算法算的比。沒有這個，我們只能說「可能有偏差」，
不能說偏差至少是 5 分鐘還是 120 分鐘——而報告裡那兩句話的份量完全不同。

⚠️ **是下界不是估計值。** 只看影片分不出「安靜的清醒」（醒著躺著不動），
   文獻要判睡/醒一律靠 PSG（腦波）。所以這裡量到的是
   *video-detectable WASO*，真值只會更高不會更低。
...
# ═══════════════════════════════════════════════════════════════════
# ⚠️ 三個標籤的意思（寫進工作單檔頭，標註時看得到）
# ═══════════════════════════════════════════════════════════════════
# 最重要的一條：**`asleep` 的意思是「沒有清醒的證據」，不是「證明睡著」。**
# 一個人醒著躺著不動，在紅外線影片上跟睡著幾乎一模一樣——這不是解析度
# 不夠，是這個模態本身看不到。文獻要判睡/醒一律靠 PSG（腦波），
# 只看影片的行為判讀在原理上就分不出「安靜的清醒」。
#
# → 所以這支估出來的是 **video-detectable WASO，是真 WASO 的下界**。
#   報告要寫「至少醒著 N 分鐘」，不能寫「醒著 N 分鐘」。
#   （這跟手機那條路的限制是同一個：兩者都看不到安靜的清醒。
#     差別在影片至少看得到「起身、開螢幕」這些手機關著時也發生的事。）
LABEL_RULES = [
    ("awake",
...
def plan(path):
    ...
    with out.open("w", encoding="utf-8", newline="") as fh:
        ...
        w.writerow(["# ⚠️ `asleep` 的意思是「沒有清醒的證據」，不是「證明睡著」。"
                    ...
                    "所以估出來的是真 WASO 的**下界**"])
        w.writerow([f"# ⚠️ 看的是一小段不是一幀——單一幀連呼吸起伏都看不到。"
...
def score(csv_path, worksheet):
    ...
    print("─" * 74)
    print(f"  假設 WASO=0 會把睡眠效率**至少高估 {p*100:.1f} 個百分點**"
