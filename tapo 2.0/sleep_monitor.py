...
def load_config():
    ...
    if system == 'Windows':
        default_ffmpeg = r"C:\Users\USER\AppData\Local\Microsoft\WinGet\Links\ffmpeg.exe"
    ...
    default_config = {
        ...
        "video_extend_cooldown": int(get_env_var('VIDEO_EXTEND_COOLDOWN', '10')),
        "max_timeline_events": int(get_env_var('MAX_TIMELINE_EVENTS', '500')),
        "enable_auto_cleanup": get_env_var('ENABLE_AUTO_CLEANUP', 'False').lower() == 'true',
        "min_valid_events": int(get_env_var('MIN_VALID_EVENTS', '5')),
        "ffmpeg_path": get_env_var('FFMPEG_PATH', default_ffmpeg),
        "ffprobe_path": get_env_var('FFPROBE_PATH', default_ffmpeg.replace('ffmpeg', 'ffprobe')),
        "video_codec": get_env_var('VIDEO_CODEC', default_video_codec),
        "headless_mode": get_env_var('HEADLESS_MODE', 'False').lower() == 'true',
        "db_host": get_env_var('DB_HOST', 'localhost'),
        "db_user": get_env_var('DB_USER', 'root'),
        "db_password": get_env_var('DB_PASSWORD', ''),
        "db_name": get_env_var('DB_NAME', 'sonnap'),
    }
    
    # 2. 從設定檔載入 (config.json)
    ...
    parser.add_argument('--camera', help='Camera RTSP URL')
    parser.add_argument('--headless', action='store_true', help='Headless mode (no display)')
    ...
    if args.camera:
        default_config['tapo_url'] = args.camera
    if args.headless:
        default_config['headless_mode'] = True
    if args.config:
        try:
    ...
    if args.interactive or (not args.start and not args.end and not os.path.exists(config_path)):
        ...
        user_input = input(f"結束時間 (預設 {current_end}): ").strip()
        if user_input:
            try:
                datetime.strptime(user_input, "%H:%M:%S")
        ...
        user_input = input(f"Camera RTSP URL [{current_url}]: ").strip()
        if user_input:
            default_config['tapo_url'] = user_input
...
# Video codec
VIDEO_CODEC = config.get('video_codec', 'mp4v' if platform.system() == 'Windows' else 'avc1')
...
print(f"📹 影片編碼: {VIDEO_CODEC}")
print(f"🖥️ Headless: {'是' if HEADLESS_MODE else '否'}\n")

# FFmpeg paths (from config)
FFMPEG_PATH = config.get('ffmpeg_path', r"C:\Users\USER\AppData\Local\Microsoft\WinGet\Links\ffmpeg.exe")
FFPROBE_PATH = config.get('ffprobe_path', FFMPEG_PATH.replace('ffmpeg', 'ffprobe'))
...
DB_CONFIG = {
    ...
    "database": config.get('db_name', 'sonnap')
}

# Platform-specific OpenCV settings
if platform.system() == 'Linux':
    # Linux might need additional FFmpeg options
...
def check_ffmpeg():
    ...
    print("   Windows: 下載自 https://ffmpeg.org/download.html")
    return False
